...
export const fetchAllPositionsInfo = async (userId: string) => {
    ...
    const positionsInfo = await Promise.all(
        allPositions.map(async (position) => {
            const nftMint = position.nftMint
            const positionPubKey = getPdaPersonalPositionAddress(CLMM_PROGRAM_ID, nftMint).publicKey
            const pos = await raydium.connection.getAccountInfo(positionPubKey)
            if (!pos) throw new Error('Position not found')
            const positionData = PositionInfoLayout.decode(pos.data)

            let poolInfo: ApiV3PoolInfoConcentratedItem

            if (raydium.cluster === 'mainnet') {
                poolInfo = (
                    await raydium.api.fetchPoolById({ ids: positionData.poolId.toBase58() })
                )[0] as ApiV3PoolInfoConcentratedItem
            } else {
                const data = await raydium.clmm.getPoolInfoFromRpc(positionData.poolId.toBase58())
            ...
            const priceLower = TickUtils.getTickPrice({
                poolInfo,
                tick: positionData.tickLower,
                baseIn: true,
            })
            const priceUpper = TickUtils.getTickPrice({
                poolInfo,
                tick: positionData.tickUpper,
                baseIn: true,
            })
            ...
            const tickLowerState =
                tickArrayLower.ticks[TickUtils.getTickOffsetInArray(positionData.tickLower, poolInfo.config.tickSpacing)]
            const tickUpperState =
                tickArrayUpper.ticks[TickUtils.getTickOffsetInArray(positionData.tickUpper, poolInfo.config.tickSpacing)]
            const rpcPoolData = await raydium.clmm.getRpcClmmPoolInfo({ poolId: positionData.poolId })
            const tokenFees = PositionUtils.GetPositionFeesV2(rpcPoolData, positionData, tickLowerState, tickUpperState)
            const rewards = PositionUtils.GetPositionRewardsV2(rpcPoolData, positionData, tickLowerState, tickUpperState)
